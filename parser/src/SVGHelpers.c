...
char *getFieldData(char *atrString, char *field) {
    if (atrString == NULL || field == NULL) {
        return NULL;
    }
    
    char *breakPt;
    char *data = malloc(sizeof(char) * (strlen(atrString) + 1));
    strcpy(data, atrString);
    breakPt = strtok(data, "\"");
    while (strcmp(breakPt, "{") == 0) {
        breakPt = strtok(NULL, "\"");
    }
    
    while (breakPt != NULL && strcmp(breakPt, field) != 0) {
        breakPt = strtok(NULL, ",");        
        breakPt = strtok(NULL, "\"");
    }
    ...
    char *data2 = malloc(sizeof(char) * (strlen(breakPt) + 1));
    strcpy(data2, breakPt);    
    strcpy(data, data2);

    if (data2[0] == '\"' && data2[strlen(data2) - 1] == '\"') {
        breakPt = strtok(data2, "\"");
        
        if (breakPt == NULL || strlen(breakPt) == 0) {
            strcpy(data, "");
        }
        else {
            strcpy(data, breakPt);
...
void fetchUnits(char *unitss, char *data, float *value) {
    if (data == NULL || unitss == NULL || (!isdigit(data[0]) && data[0] != '.')) {
        return ; 
    }
    char *unitValue = NULL; 
    int i = 0;
    for (i = 0; i < strlen(data); i++) {
        if (!isdigit(data[i]) && data[i] != '.') {
            unitValue = data + sizeof(char) * i;
            break;
        }
    }
    if (unitValue != NULL) {
        strcpy(unitss, unitValue);
    }
    unitValue = malloc(sizeof(char) * (strlen(data) + 1));
    strcpy(unitValue, data); 
    unitValue[i] = '\0';
    *value = atof(unitValue);
    free(unitValue);
